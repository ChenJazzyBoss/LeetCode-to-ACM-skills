...
系统 SHALL 以目录结构存储每道题目，每个题目目录包含 `problem.md`（ACM 格式题面）、`solution_ref.py`（参考解）和 `tests/` 子目录（测试用例文件对）。题目目录命名格式为 `<三位数题号>-<英文短名>`，如 `001-two-sum`、`015-3sum`。每道题 MUST 包含至少 3 组测试用例（基础用例、边界用例、随机用例），每组由 `.in`（输入）和 `.out`（预期输出）文件对组成。参考解 SHALL 是经过验证的正确 ACM 格式 Python 代码，用于保证测试用例预期输出的正确性。
