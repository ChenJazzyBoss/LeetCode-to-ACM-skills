...
2. **建代码文件**：`<题号>-<slug>.py`，文件头以注释形式嵌入**题面（题目描述/输入输出格式/样例/数据范围）**，正文留 `# ---- TODO: 在下面写你的代码 ----` 空区，让用户自己写。
   - **🚫 绝不贴核心代码 / 解法代码**：模板里**只能写题面和算法方向提示（文字）**，**禁止**贴任何该题的解题代码、循环结构、转移方程代码、完整逻辑。否则用户照抄就 AC，失去训练意义。
   - **算法方向提示（文字，非代码）**：可以写"用双指针""单调栈存下标""前缀积×后缀积"这类**思路方向**，但不要写出 `while l<r: ...` 这种具体代码。
   - **读写提示分阶段**：用户刚开始练 ACM（前 2-3 题）时，可在题面注释里**口头**提"读整数用 int(input())、读数组用 map+split"；观察到用户能独立写读写后，连这类口头提示也省略，只留题面。判断"熟悉"信号：连续 2-3 题读写一次写对、没问读写问题。
   - **何时才能给代码**：只有当用户**明确说"给我答案/我不会了，看答案"**时，才在对话框里给完整解法（且记录为"学习版"）。评判时绝不主动给代码。
